...
# Définir fichiers de configuration
$statusFile = "$PSScriptRoot\..\data\status.json" # Centralized status file
$monitoredFolders = @{}
$processId = $PID # Get current process ID
$startTime = Get-Date # Get start time

# Vérifier les dossiers fournis
if ($null -eq $FoldersToMonitor -or $FoldersToMonitor.Count -eq 0) {
    Write-Host "No folders to monitor" -ForegroundColor Yellow
    exit
}
...
# Initialisation des dossiers à surveiller
...
function Monitor-Folders {
    while ($true) {
        try {
            $keys = $monitoredFolders.Keys | ForEach-Object { $_ }
